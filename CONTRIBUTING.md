...
`data/comparison.json` 保存逐任务成绩，`data/methods.json` 保存方法实现的双语介绍和固定比较组。新增成绩时核对数据集、样本数、划分、指标、均值与标准差及原始出处；方法名称必须明确到具体实现。

平均排名只在固定参评方法和共同任务上计算；并列取平均名次，任务等权。改变参评范围时更新比较组 ID、任务列表及排除原因。缺失成绩会使构建失败，不会自动缩小任务集或补入数值。维护后运行构建、一致性检查和 `python -m unittest discover -s scripts -p 'test_*.py'`。方法汇总页与两份排名 CSV 由脚本生成。
