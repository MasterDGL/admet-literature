...
`data/datasets.json` 保存数据集字典；`data/comparison.json` 保存具体实现、数据集、协议、指标、均值、标准差、来源和查询日期。`source_kind` 区分榜单与论文成绩，`source_location` 记录原文表号。构建脚本生成双语说明和 CSV，并检查数据规模、指标和排序方向。更新一组比较时一起检查该数据集的所有行，保持协议与核对日期一致。
