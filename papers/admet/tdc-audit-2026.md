...
审计 10 个领先方法，分环境、泄漏、验证/测试重叠和复现阶段。结论属于审计作者，受版本限制。

为辨别可信 SOTA 提供重要问题清单。它仍是预印本，失败原因也不同；不能把环境安装失败等同于模型无效，更不能声称本次已经复现其审计。当前仓库更新可能晚于初版论文。

## 代码与证据

[代码或项目入口](https://github.com/receptor-ai/tdc-admet-bench)
