...
训练集五折交叉验证调参；报告召回、特异度、平衡准确率和筛选富集。86%/94% 是阳性召回，不是全体样本准确率，也不是 TDC hERG 榜单成绩。

论文利用测试集筛选表现选择对接姿势打分方案，不能称完全未参与选择的盲测；高召回仍伴随误报。预测流程需要对接及结构指纹；代码仓库已归档，不代表结果失效。

## 代码与证据

[代码或项目入口](https://github.com/vktrannguyen/HERGAI)

代码状态：作者代码与数据入口存在；仓库于 2026-01-01 归档，未独立复现。复现状态：本仓库未独立复现实验。
